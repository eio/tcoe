...
let currentState = "applyCurlNoise";
...
function animateParticles(geometry) {
  ...
  function animate() {
    for (let i = 0; i < positionAttribute.count; i++) {
      ...
      if (currentState === "moveTowardsOrigin") {
        // Move particles towards the scene origin
        const direction = new THREE.Vector3(0, 0, 0).sub(new THREE.Vector3(x, y, z)).normalize();
        positionAttribute.setXYZ(i, x + direction.x * 0.01, y + direction.y * 0.01, z + direction.z * 0.01);
      } else if (currentState === "applyCurlNoise") {
...
// Handle mouse click to toggle between moving particles and applying curl noise
...
window.addEventListener('click', () => {
  if (!isResettingParticles) {
    if (currentState === "moveTowardsOrigin") {
      ...
      currentState = "moveTowardsOrigin";
    }
  }
});
